...
- `@forinda/kickjs` peer range is `>=8.6.0 <9.0.0` for both packages (it had no upper bound). The MCP SDK peer is `^1.30.0`, the version the adapter is built and tested against (was `^1.0.0`).
