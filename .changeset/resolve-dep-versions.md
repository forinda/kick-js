...
Packages whose major the generated code is written against stay capped to that major: `vite`, `typescript`, the HTTP engines (`express`, `fastify`, `@fastify/middie`, `h3`, `serve-static`) and the schema libraries (`zod`, `valibot`, `yup`). Everything else tracks `latest`. When `npm view` answers nothing — offline, registry down — each package falls back to the range it shipped with, so a scaffold without network still installs.
