...
describe('generate()', () => {
  it('creates a migration on first run', async () => {
    ...
    // Immutable provenance banner — review state lives in meta.json only.
    expect(upSql).toMatch(/^-- Generated by @forinda\/kickjs-db v[\w.-]+/)
    expect(upSql).not.toContain('-- REVIEWED')
    ...
    expect(downSql).toMatch(/^-- Generated by @forinda\/kickjs-db v[\w.-]+/)
