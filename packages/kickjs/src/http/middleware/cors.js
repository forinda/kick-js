...
function isOriginAllowed(requestOrigin, allowed) {
    ...
    if (Array.isArray(allowed)) {
        return allowed.some((pattern) => typeof pattern === 'string' ? requestOrigin === pattern : pattern.test(requestOrigin));
    }
    return false;
}
/**
...
export function cors(options = {}) {
    const { origin = '*', methods = DEFAULT_METHODS, allowedHeaders, exposedHeaders, credentials = false, maxAge = 86400, preflight = true, } = options;
    return (req, res, next) => {
