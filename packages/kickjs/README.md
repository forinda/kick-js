...
- **Context Contributors** — replace single-purpose middleware that only sets `ctx` values. Typed `dependsOn` (typos are TS errors, not boot-time crashes), topo-sorted at startup. (`@forinda/kickjs-ws` handlers don't run contributors — a socket isn't a request.) See [Context Decorators](https://kickjs.app/guide/context-decorators.html).
- **Built-in health probes** — `GET /health/live` and `GET /health/ready` ship as a module, mounted at the root outside `apiPrefix` so a probe URL an orchestrator is configured against does not move. Readiness runs every adapter's `onHealthCheck()`. They sit inside your middleware chain, so app-wide auth applies — pass `bootstrap({ health: false })` to mount your own.
