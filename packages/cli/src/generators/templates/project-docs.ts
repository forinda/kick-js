...
it separates the entry-file cause from the test-file one.

**Diagnosis**:
1. Open \`src/index.ts\`. The **first non-\`reflect-metadata\`** import MUST be \`import './config'\`.
2. Open \`src/config/index.ts\`. It MUST run the loader as a top-level side effect — \`loadEnvFromSchema(envSchema)\` as generated by \`kick new\`, or \`loadEnv(zodSchema)\` for a bare Zod object.
3. **In a test?** \`createTestApp\` never loads \`src/index.ts\`, so the entry's import cannot run however correct it is. Add \`import '@/config'\` to the test file. Step 1 will look fine, which is what makes this one slow to spot.
4. The new key MUST be declared in the Zod schema there. \`@Value('NEW_KEY')\` won't work without a schema entry (it'll fall back to raw \`process.env\` and skip Zod coercion silently).

**Fix**: add the key to the schema; ensure the side-effect import is present in \`src/index.ts\` and in any test that reads config.
