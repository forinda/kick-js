 ...
 */
function buildSkills(pm: string, style: ModuleStyle = 'define'): KickJsSkill[] {
  // A class module is passed bare; a define module is invoked. Emitting the
  // wrong one calls a class without `new`, or hands the framework a factory.
  const moduleEntry = style === 'class' ? 'UserModule' : 'UserModule()'
  const skills: Array<{
    slug: string
    frontmatterName: string
    description: string
...
export function generateKickJsSkillFiles(
  ...
  const skills = buildSkills(pm, style)
