...
describe('InMemoryProjectRepository', () => {
  ...
  it('findById returns the correct project', async () => {
    const created = await repo.create({ workspaceId: 'ws1', name: 'Beta', key: 'BET' })
  ...
  it('findByWorkspace returns projects for given workspace', async () => {
  ...
  it('incrementTaskCounter increments counter and returns key', async () => {
    const project = await repo.create({ workspaceId: 'ws1', name: 'Counter', key: 'CNT' })
    const result = await repo.incrementTaskCounter(project.id)

    expect(result.taskCounter).toBe(1)
    expect(result.key).toBe('CNT')
  })

  it('delete removes the project', async () => {
    const project = await repo.create({ workspaceId: 'ws1', name: 'ToDelete', key: 'DEL' })
