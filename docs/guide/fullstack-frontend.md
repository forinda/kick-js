...
`kick new --template fullstack` asks who scaffolds `web/`. Pick **Delegate to create-vite** and the CLI hands the directory to `create-vite` — you choose React, Vue, Svelte, Solid, whatever it offers — and then wires **nothing**. Your app is yours; this page is the four steps that connect it to the API.
...
`kick typegen` (run by `kick dev`, and by the root `build` script) writes `server/.kickjs/types/kick__client.d.ts`: the resolved map of every route, as an ambient `KickClientApi` namespace. Add it to the web app's `tsconfig.json`:
...
Frameworks that own their tsconfig (Vue's `tsconfig.app.json`, SvelteKit's generated config) take the same entry in whichever file compiles `src`.
...
Nothing changes. [`kick build:netlify` and `kick build:vercel`](./serverless.md#build-it-with-one-command) publish `web/dist`, which is where Vite builds by default in every framework template. If your framework builds somewhere else (SvelteKit's `build/`, for instance), point the deploy config at it:

```ts
// server/kick.config.ts
export default defineConfig({
