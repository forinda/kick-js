...
Every feature in a KickJS application is organized as a module. Modules implement the `AppModule` interface from `@kickjs/core` and are responsible for registering DI bindings and declaring routes.

## The AppModule Interface

```ts
import type { Container } from '@kickjs/core'
